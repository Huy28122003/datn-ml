...
def main():
    print("📈 ĐANG SINH BIỂU ĐỒ BÁO CÁO TỪ FILE JSON...")
    if not os.path.exists(SUMMARY_PATH):
        print(f"❌ LỖI: Không tìm thấy file {SUMMARY_PATH}")
        return
    ...
    for model_name, data in summary_json.items():
        if 'holdout' in data:
            ...
            # Backwards compatibility with standard flat JSON format
    ...
    print("🎉 HOÀN THÀNH SINH BIỂU ĐỒ BÁO CÁO THÀNH CÔNG!")
