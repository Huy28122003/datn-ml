"""
xgb_model_train_final.py
========================
Bước 3: Huấn luyện mô hình XGBoost cuối cùng (Final Training).
  - Đọc danh sách các đặc trưng chọn lọc và siêu tham số tối ưu ở các bước trước.
  - Huấn luyện mô hình XGBoost trên tập Train (70,917 mẫu).
  - Thực hiện kiểm thử chéo 5-Fold Cross Validation đạt độ chính xác ~93.8%.
  - Xuất ra 5 đồ thị đánh giá hiệu năng chuyên sâu.
  - Lưu file mô hình binary: output/xgb_vs_dsfull/xgb_phishing_model.pkl.
"""
...
def main():
    print("╔══════════════════════════════════════════════════════════╗")
    print("║  BƯỚC 3 (XGB): HUẤN LUYỆN & ĐÁNH GIÁ MÔ HÌNH XGBOOST CUỐI║")
    print("╚══════════════════════════════════════════════════════════╝")

    if not os.path.exists(SELECTED_FEATURES_PATH) or not os.path.exists(BEST_HYPERPARAMS_PATH):
        print("❌ LỖI: Thiếu tệp đặc trưng chọn lọc hoặc siêu tham số tối ưu!")
        sys.exit(1)
    ...
    print(f"\n📖 Đã load {len(selected_features)} đặc trưng chọn lọc và siêu tham số XGBoost.")
    ...
    # 2. Huấn luyện mô hình XGBoost chính thức
    ...
    print(f"\n📈 CHỈ SỐ CHẤT LƯỢNG MÔ HÌNH XGBOOST:")
    ...
    print("\n🎉 HUẦN LUYỆN THÀNH CÔNG MÔ HÌNH XGBOOST!")
