...
def main():
    if not os.path.exists(SELECTED_FEATURES_PATH) or not os.path.exists(BEST_HYPERPARAMS_PATH):
        print("Loi: Thieu file dac trung/params!")
        sys.exit(1)
    ...
    print(f"Loaded {len(selected_features)} dac trung.")
    ...
    print(f"Precision: {report_dict['1']['precision']:.4%}")
    print(f"Recall:    {report_dict['1']['recall']:.4%}")
    print(f"F1-Score:  {report_dict['1']['f1-score']:.4%}")

    # huyny - 5-Fold CV kiem dinh
    print("Run 5-Fold CV...")
    ...
    print(f"Mean CV: {cv_scores.mean():.4%}")
    ...
    plt.title('Hybrid RF - Ma tran nham lan (Confusion Matrix)', fontsize=14, pad=15)
    plt.xlabel('Nhan du doan boi Model', fontsize=12)
    plt.ylabel('Nhan thuc te (Ground Truth)', fontsize=12)
    plt.tight_layout()
    ...
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.title('Hybrid RF - Duong cong ROC', fontsize=14, pad=15)
    plt.xlabel('Ty le canh bao sai (False Positive Rate)', fontsize=12)
    plt.ylabel('Ty le phat hien dung (True Positive Rate)', fontsize=12)
    ...
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.title('Hybrid RF - Duong cong Precision-Recall', fontsize=14, pad=15)
    plt.xlabel('Ty le phat hien dung (Recall)', fontsize=12)
    plt.ylabel('Do chinh xac lop canh bao (Precision)', fontsize=12)
    ...
    plt.axhline(y=cv_scores.mean(), color='red', linestyle='--', lw=1.5, label=f'Trung binh = {cv_scores.mean():.4%}')
    plt.ylim([min(cv_scores) - 0.01, 1.0])
    plt.title('Hybrid RF - Tinh on dinh qua 5-Fold CV', fontsize=14, pad=15)
    plt.ylabel('Do chinh xac (Accuracy)', fontsize=12)
    plt.legend(fontsize=11)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'rf_cross_validation_scores.png'), dpi=150)
    plt.close()

    print("Tinh Learning Curve...")
    ...
    plt.plot(train_sizes, train_scores_mean, 'o-', color="r", label="Do chinh xac tap Train", lw=2)
    plt.plot(train_sizes, val_scores_mean, 's-', color="g", label="Do chinh xac tap Validation", lw=2)
    plt.title("Hybrid RF - Duong cong hoc tap", fontsize=14, pad=15)
    plt.xlabel("Mau huan luyen", fontsize=12)
    plt.ylabel("Do chinh xac", fontsize=12)
    ...
    plt.title('Hybrid RF - Bao cao phan loai (Classification Report)', fontsize=12, pad=12)
    plt.tight_layout()
    ...
    print(f"Saved summary: {TRAINING_SUMMARY_PATH}")
