...
def main():
    if not os.path.exists(SELECTED_FEATURES_PATH):
        print(f"Loi: Thieu {SELECTED_FEATURES_PATH}")
        sys.exit(1)
    ...
    # huyny - lay mau chay grid search
    print("Lay mau 40k dong...")
    ...
    print("Khao sat n_estimators...")
    n_estimators_range = [50, 100, 150, 200, 250]
    train_accuracies = []
    test_accuracies = []

    for n in n_estimators_range:
        print(f"Check n_estimators = {n}")
    ...
    plt.plot(n_estimators_range, train_accuracies, 'o-', label='Do chinh xac Train', color='#1f77b4', linewidth=2)
    plt.plot(n_estimators_range, test_accuracies, 's-', label='Do chinh xac Test', color='#ff7f0e', linewidth=2)
    plt.title('Hybrid - Khao sat n_estimators vs Do chinh xac', fontsize=14, pad=15)
    plt.xlabel('So luong cay (n_estimators)', fontsize=12)
    ...
    print(f"CV Accuracy: {best_score:.4%}")
    ...
    plt.title('Hybrid - Do chinh xac Cross-Validation theo tham so', fontsize=12, pad=12)
    ...
    print(f"Luu params: {BEST_HYPERPARAMS_PATH}")
